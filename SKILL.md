...
Seed and mutate live rows with MCP `gabriel_upsert_list_records` (REST `POST /api/gateway/data-lists/{listId}/records`) and verify with `gabriel_get_list_records`. Pipeline-backed lists stamp `data._workflowState[pipelineId].stage` onto the initial stage (or an explicit `stageId`) so Canvas transitions can find the rows. Catalog lists such as Pokemon have no pipeline — pass `upsertKey` (for example `name`) and column fields only.
